...
### 3.3. データベース
- **種類**: SQLite
- **ライブラリ**: better-sqlite3 v12.6.2

### 3.4. デプロイ環境
- ローカル環境、Render.com、Docker、VPS
- 実行方法: npm scripts / Docker / Render.com
...
#### FR-106: 画像添付
- **要件ID**: FR-106
- **要件概要**: 日記エントリーに画像を添付できるようにする
- **優先度**: 低
- **対応状況**: 未着手
- **起票日**: 2026-01-31
- **詳細説明**: 日記エントリーに画像ファイルを添付し、エントリー表示時に画像が表示されるようにする。
- **受け入れ基準**:
    - 日記作成・編集時に画像アップロードUIが存在すること
    - 添付された画像がエントリー表示時に正しく表示されること
...
| 1.2.0      | 2026-02-04 | FR-107（ダークモード）の対応状況を実装済みに更新 |
| 1.3.0      | 2026-02-23 | 技術スタックの詳細情報を追記。FR-106（画像添付）を追加。CLAUDE.mdより統合 |
| 1.4.0      | 2026-03-04 | FR-103（カレンダービュー）実装完了 |
